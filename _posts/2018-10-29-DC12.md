...
For this exercise, we will construct a plot showing four time series stocks on the same axes. The time series in question are represented in the session using the identifiers aapl, ibm, csco, and msft. We'll generate a single plot showing all the time series on common axes with a legend.
...
We can easily slice subsets corresponding to different time intervals from a time series. In particular, we can use strings like '2001:2005', '2011-03:2011-12', or '2010-04-19:2010-04-30' to extract data from time intervals of length 5 years, 10 months, or 12 days respectively.
...
* We can use partial strings or datetime objects for indexing and slicing from time series.

For this exercise, we will use time series slicing to plot the time series aapl over its full 11-year range and also over a shorter 2-year range. We'll arrange these plots in a 2×1 grid of subplots.
...
In this exercise, we will use the same time series aapl from the previous exercise and plot tighter views of the data.
...
Remember, rather than comparing plots with subplots or overlayed plots, we can generate an inset view directly using plt.axes(). In this exercise, we'll reproduce two of the time series plots from the preceding two exercises. Our figure will contain an inset plot to highlight the dramatic changes in AAPL stock price between November 2007 and April 2008 (as compared to the 11 years from 2001 to 2011).
...
In this exercise, we will plot pre-computed moving averages of AAPL stock prices in distinct subplots.

* The time series aapl is overlayed in black in each subplot for comparison.
* The time series mean_30, mean_75, mean_125, and mean_250 have been computed for us (containing the windowed averages of the series aapl computed over windows of width 30 days, 75 days, 125 days, and 250 days respectively).
...
Having plotted pre-computed moving averages of AAPL stock prices on distinct subplots in the previous exercise, we will now plot pre-computed moving standard deviations of the same stock prices, this time together on common axes.
...
A histogram of a continuous random variable is sometimes called a Probability Distribution Function (or PDF). The area under a PDF (a definite integral) is called a Cumulative Distribution Function (or CDF). The CDF quantifies the probability of observing certain pixel intensities. Our task here is to plot the PDF and CDF of pixel intensities from a grayscale image. We will use the [grayscale image of Hawkes Bay, New Zealand](https://commons.wikimedia.org/wiki/File:Unequalized_Hawkes_Bay_NZ.jpg) (originally by Phillip Capper, modified by User:Konstable, via Wikimedia Commons, [CC BY 2.0](http://creativecommons.org/licenses/by/2.0)). This time, the 2D array image will be pre-loaded and pre-flattened into the 1D array pixels for you.
...
[Histogram equalization](https://en.wikipedia.org/wiki/Histogram_equalization) is an image processing procedure that reassigns image pixel intensities. The basic idea is to use interpolation to map the original CDF of pixel intensities to a CDF that is almost a straight line. In essence, the pixel intensities are spread out and this has the practical effect of making a sharper, contrast-enhanced image. This is particularly useful in astronomy and medical imaging to help us see more features.

For this exercise, we will again work with the [grayscale image of Hawkes Bay, New Zealand](https://commons.wikimedia.org/wiki/File:Unequalized_Hawkes_Bay_NZ.jpg) (originally by Phillip Capper, modified by User:Konstable, via Wikimedia Commons, [CC BY 2.0](http://creativecommons.org/licenses/by/2.0)). Notice the sample code produces the same plot as the previous exercise. Our task is to modify the code from the previous exercise to plot the new equalized image as well as its PDF and CDF.
...
This exercise resembles the last in that we will plot histograms from an image. This time, we will use [a color image of the Helix Nebula as seen by the Hubble and the Cerro Toledo Inter-American Observatory](http://imgsrc.hubblesite.org/hu/db/images/hs-2004-32-b-small_web.jpg). The separate RGB (red-green-blue) channels will be extracted for you as two-dimensional arrays red, green, and blue respectively. We will plot three overlaid color histograms on common axes (one for each channel) in a subplot as well as the original image in a separate subplot.
...
Rather than overlaying univariate histograms of intensities in distinct channels, it is also possible to view the joint variation of pixel intensity in two different channels. For this final exercise, we will use the same [color image of the Helix Nebula as seen by the Hubble and the Cerro Toledo Inter-American Observatory](http://imgsrc.hubblesite.org/hu/db/images/hs-2004-32-b-small_web.jpg). The separate RGB (red-green-blue) channels will be extracted for you as one-dimensional arrays red_pixels, green_pixels, & blue_pixels respectively.
...
## Datesets:
* [Automobile miles per gallon](https://assets.datacamp.com/production/course_1761/datasets/auto-mpg.csv)
* [Percentage of bachelor's degrees awarded to women in the USA](https://assets.datacamp.com/production/course_1761/datasets/percent-bachelors-degrees-women-usa.csv)
* [Stocks](https://assets.datacamp.com/production/course_1761/datasets/stocks.csv)
